...
class LMDBRecordReader : public ReaderBase {
 public:
  LMDBRecordReader(const string& node_name, Env* env)
      : ReaderBase(strings::StrCat("LMDBRecordReader '", node_name, "'")),
        env_(env),
        mdb_env_(nullptr),
        mdb_txn_(nullptr),
        mdb_cursor_(nullptr) {}
...
class LMDBRecordReaderOp : public ReaderOpKernel {
 public:
  explicit LMDBRecordReaderOp(OpKernelConstruction* context)
      : ReaderOpKernel(context) {
    Env* env = context->env();
    SetReaderFactory([this, env]() {
      return new LMDBRecordReader(name(), env);
    });
  }
};

REGISTER_KERNEL_BUILDER(Name("LMDBRecordReader").Device(DEVICE_CPU),
                        LMDBRecordReaderOp);
